...
import { Layout, Footer, Container, Card, CardContent } from '@/layout'
  ...
  const onRoomClick = React.useCallback(
    (id: string, hasPassword?: boolean) => {
      return () => {
        ...
        } else router.push(`/${id}/${props.finalsStarted ? 'finals' : 'groups'}`)
      }
    },
    ...
    (password: string) => {
      axios
        ...
        .then((response) => {
          ...
          setPasswordModalId('')
          if (allowed) {
            router.push(`/${passwordModalId}/${props.finalsStarted ? 'finals' : 'groups'}`)
          }
        })
    },
    ...
    <Layout backgroundImage={`/${props.userRanking?.background}.png`}>
      <DesktopHeader userRanking={props.userRanking}>
        {props.registeredProdes <= 1 && (
          <Button invert href="/">
            {props.finalsStarted ? i18n.buttonLabelFinalsPhase : i18n.buttonLabelGroupPhase}
          </Button>
        )}
      ...
      <MobileHeader finalsStarted={props.finalsStarted} userRanking={props.userRanking} create groups />
            ...
            <Table
              ...
              columns={[
                ...
                },
                {
                  ...
                  width: '250px',
                  accesor: (row) => (
                    <div style={{ display: 'flex', placeContent: 'center end' }}>
                      {row.hasPassword && <LockIcon open={row.open} />}
                      <Button
                        variant="transparent"
                        onClick={onRoomClick(row.id, row.open ? false : row.hasPassword)}
                      >
                        {row.alreadyJoin ? i18n.roomsProdeListRedirectLabel : i18n.roomsProdeListJoinLabel}
                      </Button>
                    </div>
                  ),
                },
